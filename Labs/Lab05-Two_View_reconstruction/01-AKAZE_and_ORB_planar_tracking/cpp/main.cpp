...
const double akaze_thresh = 3e-4;       // AKAZE detection threshold set to locate 1000 key points
const double ransac_thresh = 2.5f;       // RANSAC inlier threshold
const double nn_match_ratio = 0.8f;     // Nearest-neighbour matching ratio
const int bb_min_inliers = 100;         // Minimal number of inliers to draw bounding box
const int stats_update_period = 10;     // On-screen statistics are update every 10 frames
...
int main(int argc, char** argv) {
    string video_name = "../../../../Data/robot.mp4";
    ...
    Mat frame;
    namedWindow("ROBOT.MP4", WINDOW_NORMAL);
    cout << "\nPress any key to stop the video and select a bounding box" << endl;
    while (waitKey(1) < 1)
    {
        video_in >> frame;
        cv::resizeWindow("ROBOT.MP4", frame.size());
        imshow("ROBOT.MP4", frame);
    }
    vector<Point2f>bb;
    cv::Rect uBox = cv::selectROI("ROBOT.MP4", frame);
